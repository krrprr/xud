...
class Pool extends EventEmitter {
  ...
  private bindNodeList = () => {
    this.nodes.on('node.ban', (nodePubKey: string, events: ReputationEventInstance[]) => {
      ...
      if (peer) {
        ...
        return peer.close(this.nodePubKey, DisconnectionReason.Banned, JSON.stringify(lastNegativeEvents));
      }
      return;
    });
  }
  ...
  public closePeer = async (nodePubKey: string, reason?: DisconnectionReason, reasonPayload?: string) => {
    const peer = this.peers.get(nodePubKey);
    if (peer) {
      await peer.close(this.nodePubKey, reason, reasonPayload);
  ...
  private validatePeer = async (peer: Peer): Promise<void> => {
    ...
    if (peerPubKey === this.nodePubKey) {
      await peer.close(this.nodePubKey, DisconnectionReason.ConnectedToSelf);
    ...
    if (!semver.valid(peer.version)) {
      await peer.close(this.nodePubKey, DisconnectionReason.MalformedVersion);
    ...
    if (semver.compare(peer.version, minCompatibleVersion) === -1) {
      await peer.close(this.nodePubKey, DisconnectionReason.IncompatibleProtocolVersion);
    ...
    if (!this.connected) {
      ...
      await peer.close(this.nodePubKey, DisconnectionReason.NotAcceptingConnections);
    ...
    if (this.nodes.isBanned(peerPubKey)) {
      ...
      await peer.close(this.nodePubKey, DisconnectionReason.Banned);
    ...
    if (this.peers.has(peerPubKey)) {
      ...
      await peer.close(this.nodePubKey, DisconnectionReason.AlreadyConnected);
  ...
  private closePeers = () => {
    ...
    for (const peer of this.peers.values()) {
      closePromises.push(peer.close(this.nodePubKey, DisconnectionReason.Shutdown));
    }
  ...
  private closePendingConnections = () => {
    ...
    for (const peer of this.pendingOutboundPeers.values()) {
      closePromises.push(peer.close(this.nodePubKey));
    }
